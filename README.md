...
`graceful-k8s-shutdown` - учебный прикладной модуль для небольших HTTP-приложений на Go, развертываемых в Kubernetes. Модуль согласует readiness-пробу, `preStop`, `SIGTERM`, учет активных HTTP-запросов и асинхронных операций, закрытие ресурсов и структурированный журнал завершения.
...
Клиент запускает 50 HTTP-запросов с интервалом между стартами. Каждый запрос выполняется в отдельной goroutine, поэтому операции частично перекрываются по времени.
